...
PileupBundle::PileupBundle(PileupBundle&& o) noexcept
    : pos{o.pos}, span{o.span}, ref_seq{std::move(o.ref_seq)},
      storage{o.storage}, data{std::move(o.data)}
{
    o.storage = nullptr;
}
...
)
{
    ...
    if (data.empty()) {
        b.span = {0, 0};  // unknown implications if this is later used to fetch reference
    }
    else {
        b.span = {data.front()->b->core.pos, data.back()->b->core.pos};
    }

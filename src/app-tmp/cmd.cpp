...
{
  return std::format(
      "frame: {}", ctx::get<GlobalContext>().data.frame
  );
}
static std::unordered_map<std::string_view, std::string (*)()>
...
std::pair<std::string_view, std::string_view> split_first_space(
    std::string_view s
)
{
  if (s.empty()) {
...
{
  auto& show_requested =
      ctx::get<GlobalContext>().conf.debug_request;
  ...
  return {
      true, std::format("Showing debug properties: {}", new_reqs)
  };
}
...
CmdResult pileup_show(std::string_view names)
{
  ...
  return {
      true, std::format("Showing query properties: {}", new_reqs)
  };
}
...
static std::unordered_map<
    std::string_view, CmdResult (*)(std::string_view)>
    CMD_REGISTRY{
        {"q", &quit},
        {"quit", &quit},

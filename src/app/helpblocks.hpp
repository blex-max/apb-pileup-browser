...
inline constexpr auto app = std::to_array<std::string_view> ({
    ...
    "  Commands are typed into an in-app command line    ",
    ...
    "  including query examples - run                    ",
    "    `apb --manual > MANUAL.txt`                     ",
    "  at the CLI to get a copy.                         ",
    "                                                    ",
    "  All coordinate data is displayed in 1-based       ",
    "  closed coordinates.                               ",
});
static_assert (
...
inline constexpr auto tableColumns = std::to_array<std::string_view> ({
    " TABLE REFERENCE                                ",
    "                                                ",
    ...
    "  `end`:                                        ",
    "      1-based rightmost mapping position        ",
    "  `mapq`:                                       ",
    "      mapping quality                           ",
    "  `basequal`:                                   ",
    "      Phred base quality at the pileup position ",
    "  `qpos`:                                       ",
    "      1-based offset into `seq`/`qual` to reach ",
...
static_assert (
    !tableColumns.empty() &&
        std::ranges::all_of (
            tableColumns,
            [] (std::string_view r) {
              return r.size() == tableColumns.front().size();
            }
        ),
    "rows must be of the same width"
);

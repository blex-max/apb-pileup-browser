...
class text_style {
 ...
 public:
  ...
  friend FMT_CONSTEXPR auto operator| (
      text_style lhs, text_style rhs
  ) -> text_style
  {
    return lhs |= rhs;
  }

  FMT_CONSTEXPR auto operator== (text_style rhs) const noexcept
      -> bool
  {
  ...
  FMT_CONSTEXPR auto get_foreground() const noexcept
      -> detail::color_type
  {
    FMT_ASSERT (
        has_foreground(),
  ...
  FMT_CONSTEXPR auto get_background() const noexcept
      -> detail::color_type
  {
    FMT_ASSERT (
        has_background(),
  ...
  {
    FMT_ASSERT (
        has_emphasis(), "no emphasis specified for this style"
    );
 ...
 private:
  ...
  friend FMT_CONSTEXPR auto fg (
      detail::color_type foreground
  ) noexcept -> text_style;

  friend FMT_CONSTEXPR auto bg (
...
FMT_CONSTEXPR inline auto operator| (
    emphasis lhs, emphasis rhs
) noexcept -> text_style
{
...
struct ansi_color_escape {
  FMT_CONSTEXPR ansi_color_escape (
      color_type text_color, const char* esc
  ) noexcept
  {
 ...
 private:
  ...
  static FMT_CONSTEXPR void to_esc (
      uint8_t c, Char* out, char delimiter
  ) noexcept
  {
  ...
  static FMT_CONSTEXPR auto has_emphasis (
      emphasis em, emphasis mask
  ) noexcept -> bool
  {
    return static_cast<uint8_t> (em) &
...
) noexcept -> ansi_color_escape<Char>
{
...
struct styled_arg : view {
  const T& value;
  text_style style;
  FMT_CONSTEXPR styled_arg (const T& v, text_style s)
      : value (v), style (s)
  {
  }
};
...
inline void vprint (
    FILE* f, text_style ts, string_view fmt, format_args args
)
{
  auto buf = memory_buffer();
  detail::vformat_to (buf, ts, fmt, args);
  print (
      f, FMT_STRING ("{}"), string_view (buf.begin(), buf.size())
  );
}
...
template <typename... T>
void print (
    FILE* f, text_style ts, format_string<T...> fmt, T&&... args
)
{
...
inline auto vformat (
    text_style ts, string_view fmt, format_args args
) -> std::string
{
  auto buf = memory_buffer();
  detail::vformat_to (buf, ts, fmt, args);
...
auto vformat_to (
    OutputIt out, text_style ts, string_view fmt,
    format_args args
) -> OutputIt
{
...
inline auto format_to (
    OutputIt out, text_style ts, format_string<T...> fmt,
    T&&... args
) -> OutputIt
{
...
struct formatter<detail::styled_arg<T>, Char>
  ...
  ) const -> decltype (ctx.out())
  {
    ...
    if (ts.has_emphasis()) {
      has_style = true;
      auto emphasis =
          detail::make_emphasis<Char> (ts.get_emphasis());
      out = detail::copy<Char> (
          emphasis.begin(), emphasis.end(), out
      );
    }
    if (ts.has_foreground()) {
      has_style = true;
      ...
      out = detail::copy<Char> (
          foreground.begin(), foreground.end(), out
      );
    }
    if (ts.has_background()) {
      has_style = true;
      ...
      out = detail::copy<Char> (
          background.begin(), background.end(), out
      );
    }
    ...
    if (has_style) {
      auto reset_color = string_view ("\x1b[0m");
      out = detail::copy<Char> (
          reset_color.begin(), reset_color.end(), out
      );
    }
    return out;
  }

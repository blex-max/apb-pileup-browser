...
// Check FMT_CPLUSPLUS to suppress a bogus warning in MSVC.
    ...
    (!defined(FMT_CPP_LIB_FILESYSTEM) || \
...
)
{
  if constexpr (std::is_same_v<Char, char> &&
    ...
    write_escaped_string<wchar_t> (
        std::back_inserter (buf), native
    );
    bool valid = to_utf8<wchar_t>::convert (
        quoted, {buf.data(), buf.size()}
    );
    ...
    write_escaped_string<Char> (
        std::back_inserter (quoted), p.string<Char>()
    );
  }
}
...
FMT_CONSTEXPR auto write_escaped_alternative (
    OutputIt out, const T& v, FormatContext& ctx
) -> OutputIt
{
  if constexpr (has_to_string_view<T>::value) {
    return write_escaped_string<Char> (
...
struct is_variant_like_<std::variant<Types...>>
    : std::true_type {};
...
class is_variant_formattable {
  ...
  static auto check (std::index_sequence<Is...>)
      -> std::conjunction<is_formattable<
          std::variant_alternative_t<Is, Variant>, Char>...>;

 public:
  static constexpr bool value = decltype (check (
      std::make_index_sequence<
          std::variant_size<Variant>::value>()
  ))::value;
};
...
#if FMT_USE_RTTI
inline auto normalize_libcxx_inline_namespaces (
...
) -> string_view
{
  ...
  if (demangled_name_view.starts_with ("std::")) {
    char* to = begin + 5;  // std::
    for (const char *from = to,
                    *end = begin + demangled_name_view.size();
         from < end;) {
    ...
    demangled_name_view = {
        begin, detail::to_unsigned (to - begin)
    };
  }
...
auto write_demangled_name (
    OutputIt out, const std::type_info& ti
) -> OutputIt
{
  ...
  // front of types
  name_copy.erase (
      normalize_msvc_abi_name (
          demangled_name, name_copy.begin()
      ),
      name_copy.end()
  );
  ...
  const string_view normalized_name =
      normalize_libcxx_inline_namespaces (
          name_copy, name_copy.data()
      );
  return detail::write_bytes<char> (out, normalized_name);
#else
  return detail::write_bytes<char> (
      out, string_view (ti.name())
  );
#endif
}
...
struct has_format_as<
    T, void_t<decltype (format_as (std::declval<const T&>()))>>
    : std::true_type {};
...
struct has_format_as_member<
    T, void_t<decltype (formatter<T>::format_as (
           std::declval<const T&>()
       ))>> : std::true_type {};
 ...
 public:
  ...
  ) const
  {
    ...
    if (!debug_) {
      ...
      return detail::write (
          ctx.out(), basic_string_view<Char> (s), specs
      );
    }
  ...
  ) const -> decltype (ctx.out())
  {
    ...
    if (value.has_value()) {
      ...
      if constexpr (!std::is_void<T>::value) {
        out = detail::write_escaped_alternative<Char> (
            out, *value, ctx
        );
      }
    }
...
struct formatter<
  ...
  template <typename FormatContext>
  auto format (
      const std::unexpected<E>& value, FormatContext& ctx
  ) const -> decltype (ctx.out())
  {
    auto out = ctx.out();
...
struct is_variant_like {
  static constexpr bool value =
      detail::is_variant_like_<T>::value;
};
...
struct formatter<std::error_code> {
 ...
 public:
  ...
  ) const -> decltype (ctx.out())
  {
    ...
    if (debug_) {
      detail::write_escaped_string<char> (
          std::back_inserter (quoted), str
      );
...
struct formatter<detail::bitint<N>, Char>
  ...
  template <typename Context>
  auto format (detail::bitint<N> x, Context& ctx) const
      -> decltype (ctx.out())
  {
    return formatter<long long, Char>::format (
        format_as (x), ctx
    );
  }
};
...
struct formatter<detail::ubitint<N>, Char>
  ...
  template <typename Context>
  auto format (detail::ubitint<N> x, Context& ctx) const
      -> decltype (ctx.out())
  {
...
struct formatter<std::byte, Char> : formatter<unsigned, Char> {
  FMT_CONSTEXPR static auto format_as (std::byte b)
      -> unsigned char
  {
  ...
  FMT_CONSTEXPR auto format (std::byte b, Context& ctx) const
      -> decltype (ctx.out())
  {
    return formatter<unsigned, Char>::format (
        format_as (b), ctx
    );
  }
};
 ...
 private:
  ...
  ) const -> OutputIt
  {
    if (c.real() != 0) {
      *out++ = Char ('(');
      out = detail::write<Char> (
          out, c.real(), specs, ctx.locale()
      );
      specs.set_sign (sign::plus);
      out = detail::write<Char> (
          out, c.imag(), specs, ctx.locale()
      );
    ...
    out =
        detail::write<Char> (out, c.imag(), specs, ctx.locale());
    if (!detail::isfinite (c.imag())) {
      *out++ = Char (' ');
    }
    *out++ = Char ('i');
...
struct formatter<
    ...
    enable_if_t<
        is_formattable<remove_cvref_t<T>, Char>::value &&
        !detail::has_format_as<T>::value &&
        !detail::has_format_as_member<T>::value>>
    : formatter<remove_cvref_t<T>, Char> {
  template <typename FormatContext>
  auto format (
      std::reference_wrapper<T> ref, FormatContext& ctx
  ) const -> decltype (ctx.out())
  {
    return formatter<remove_cvref_t<T>, Char>::format (
        ref.get(), ctx
    );
  }
};

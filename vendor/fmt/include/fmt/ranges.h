...
class is_tuple_like_ {
  template <
      typename U, typename V = typename std::remove_cv<U>::type>
...
struct make_integer_sequence
    : make_integer_sequence<T, N - 1, N - 1, Ns...> {};
...
template <
    typename T, typename C, bool = is_tuple_like_<T>::value>
...
FMT_CONSTEXPR void for_each (
    index_sequence<Is...>, Tuple&& t, F&& f
)
{
  using std::get;
...
template <
    typename Tuple1, typename Tuple2, typename F, size_t... Is>
void for_each2 (
    index_sequence<Is...>, Tuple1&& t1, Tuple2&& t2, F&& f
)
{
  using std::get;
  const int unused[] = {
      0, ((void)f (get<Is> (t1), get<Is> (t2)), 0)...
  };
  ignore_unused (unused);
}
...
using result_t =
    std::tuple<formatter<remove_cvref_t<T>, Char>...>;

using std::get;
template <typename Tuple, typename Char, size_t... Is>
auto get_formatters (index_sequence<Is...>) -> result_t<
...
using range_format_constant =
    std::integral_constant<range_format, K>;
...
struct format_tuple_element {
  ...
  void operator() (const formatter<T, char_type>& f, const T& v)
  {
    if (i > 0) {
      ctx.advance_to (
          detail::copy<char_type> (separator, ctx.out())
      );
    }
...
struct formatter<
 ...
 public:
      ...
      -> const Char*
  {
    ...
    ctx.advance_to (it);
    detail::for_each (
        formatters_, detail::parse_empty_specs<Char>{ctx}
    );
    return it;
  }
  ...
  {
    ctx.advance_to (
        detail::copy<Char> (opening_bracket_, ctx.out())
    );
    detail::for_each2 (
        formatters_, value,
...
using range_formatter_type =
    formatter<remove_cvref_t<Element>, Char>;
...
struct range_formatter<
 ...
 private:
  ...
  template <
      typename Output, typename It, typename Sentinel,
      typename U = T,
      FMT_ENABLE_IF (std::is_same<U, Char>::value)>
  auto write_debug_string (
      Output& out, It it, Sentinel end
  ) const -> Output
  {
    ...
    return detail::write<Char> (
        out, basic_string_view<Char> (buf.data(), buf.size()),
        specs
    );
  }
  ...
  template <
      ...
      typename U = T,
      FMT_ENABLE_IF (!std::is_same<U, Char>::value)>
  auto write_debug_string (Output& out, It, Sentinel) const
      -> Output
  {
    return out;
  }
...
struct range_format_kind
    : conditional_t<
          ...
          std::integral_constant<
              range_format, range_format::disabled>> {};
...
struct formatter<
 ...
 private:
  ...
  range_formatter<detail::uncvref_type<range_type>, Char>
      range_formatter_;
 ...
 public:
  ...
  FMT_CONSTEXPR auto format (
      range_type& range, FormatContext& ctx
...
struct formatter<
 ...
 private:
  ...
  decltype (detail::tuple::get_formatters<element_type, Char> (
      detail::tuple_index_sequence<element_type>()
  )) formatters_;
 ...
 public:
  ...
  template <typename FormatContext>
  auto format (map_type& map, FormatContext& ctx) const
      -> decltype (ctx.out())
  {
    auto out = ctx.out();
 ...
 private:
  ...
  using string_type = conditional_t<
      std::is_constructible<
          ...
          decltype (detail::range_begin (std::declval<R>())),
          decltype (detail::range_end (std::declval<R>()))>::
          value,
...
struct join_view : detail::view {
  ...
  FMT_CONSTEXPR join_view (
      It b, Sentinel e, basic_string_view<Char> s
  )
...
struct formatter<join_view<It, Sentinel, Char>, Char> {
 ...
 public:
  ...
  template <typename FormatContext>
  FMT_CONSTEXPR auto format (
      view& value, FormatContext& ctx
  ) const -> decltype (ctx.out())
  {
    using iter = conditional_t<
        std::is_copy_constructible<view>::value, It, It&>;
    iter it = value.begin;
    auto out = ctx.out();
    if (it == value.end) {
      return out;
...
struct tuple_join_view : detail::view {
  ...
  FMT_CONSTEXPR tuple_join_view (
      const Tuple& t, basic_string_view<Char> s
  )
      : tuple (t), sep{s}
  {
  }
...
template <typename Tuple, typename Char>
struct formatter<
    tuple_join_view<Tuple, Char>, Char,
 ...
 private:
  ...
  FMT_CONSTEXPR auto do_parse (
      parse_context<Char>& ctx, std::integral_constant<size_t, 0>
  ) -> const Char*
  {
    return ctx.begin();
  }
  ...
  template <size_t N>
  FMT_CONSTEXPR auto do_parse (
      parse_context<Char>& ctx, std::integral_constant<size_t, N>
  ) -> const Char*
  {
    auto end = ctx.begin();
    ...
    if (N > 1) {
      auto end1 = do_parse (
          ctx, std::integral_constant<size_t, N - 1>()
      );
      if (end != end1) {
        report_error (
  ...
  FMT_CONSTEXPR auto do_format (
      const tuple_join_view<Tuple, Char>& value,
      FormatContext& ctx, std::integral_constant<size_t, N>
  ) const -> typename FormatContext::iterator
  {
    using std::get;
    auto out =
        std::get<std::tuple_size<Tuple>::value - N> (formatters_)
            .format (
                get<std::tuple_size<Tuple>::value - N> (
                    value.tuple
                ),
                ctx
            );
...
struct formatter<
    ...
    enable_if_t<conjunction<
        ...
        bool_constant<
            range_format_kind<T, Char>::value ==
...
auto join (It begin, Sentinel end, string_view sep)
...
FMT_CONSTEXPR auto join (
    const Tuple& tuple FMT_LIFETIMEBOUND, string_view sep

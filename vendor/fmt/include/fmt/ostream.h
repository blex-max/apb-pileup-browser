...
template class file_access<
    file_access_tag, std::filebuf, &std::filebuf::_Myfile>;
...
void write_buffer (
    std::basic_ostream<Char>& os, buffer<Char>& buf
)
{
...
struct basic_ostream_formatter
      ...
      -> decltype (ctx.out())
  {
    ...
    output.imbue (
        std::locale::classic()
    );  // The default is always unlocalized.
    output << value;
    output.exceptions (
...
struct formatter<detail::streamed_view<T>, Char>
  ...
  auto format (detail::streamed_view<T> view, Context& ctx) const
      -> decltype (ctx.out())
  {
    return basic_ostream_formatter<Char>::format (
        view.value, ctx
    );
  }
};
 ...
 */
template <typename T>
constexpr auto streamed (const T& value)
    -> detail::streamed_view<T>
{
  return {value};
}

inline void vprint (
    std::ostream& os, string_view fmt, format_args args
)
{

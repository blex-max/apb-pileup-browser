...
__declspec (dllimport) BOOL __stdcall ReportEventW (
    HANDLE hEventLog, WORD wType, WORD wCategory,
    DWORD dwEventID, void* lpUserSid, WORD wNumStrings,
    DWORD dwDataSize, LPCWSTR* lpStrings, void* lpRawData
);

__declspec (dllimport) LSTATUS __stdcall RegCreateKeyExW (
...
__declspec (dllimport) LSTATUS __stdcall RegSetValueExW (
...
__declspec (dllimport) LSTATUS __stdcall RegOpenKeyExW (
    HKEY hKey, LPCWSTR lpSubKey, DWORD ulOptions,
    DWORD samDesired, HKEY* phkResult
);
__declspec (dllimport) LSTATUS __stdcall RegDeleteKeyW (
...
)
{
  return plog::GetConsoleScreenBufferInfo (
      consoleOutput,
      reinterpret_cast<_CONSOLE_SCREEN_BUFFER_INFO*> (

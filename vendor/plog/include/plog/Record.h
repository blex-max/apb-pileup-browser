...
#if !defined(_MSC_VER) || \
    _MSC_VER >            \
        1400  // MSVC 2005 doesn't understand `enableIf`, so drop all `meta`
namespace meta {
template <class T>
inline T& declval()
{
...
struct isConvertible {
  ...
  static typename enableIf<
      !!(sizeof (static_cast<To> (meta::declval<U>())) +
         sizeof (U*)),
      Yes>::type
  test (int);
...
struct isFilesystemPath {
  template <class U>
  static typename meta::enableIf<
      !!(sizeof (meta::declval<U>().preferred_separator)),
      Yes>::type
  test (int);
...
inline void operator<< (
    util::nostringstream& stream, const wchar_t* data
)
{
  ...
  std::operator<< (
      stream, util::toNarrow (data, codePage::kUTF8)
  );
#else
...
inline void operator<< (
    util::nostringstream& stream, wchar_t* data
)
{
  plog::detail::operator<< (
      stream, const_cast<const wchar_t*> (data)
  );
}

inline void operator<< (
    util::nostringstream& stream, const std::wstring& data
)
{
...
inline void operator<< (
    util::nostringstream& stream, const char* data
)
{
  data = data ? data : "(null)";
...
{
  plog::detail::operator<< (
      stream, const_cast<const char*> (data)
  );
}

inline void operator<< (
    util::nostringstream& stream, const std::string& data
)
{
  plog::detail::operator<< (stream, data.c_str());
}
...
#ifdef __cpp_char8_t
inline void operator<< (
    util::nostringstream& stream, const char8_t* data
)
{
#if PLOG_CHAR_IS_UTF8
  plog::detail::operator<< (
  ...
  plog::detail::operator<< (
      stream,
      util::toWide (
          reinterpret_cast<const char*> (data), codePage::kUTF8
      )
  );
#endif
}
...
inline void operator<< (
    util::nostringstream& stream, const std::pair<T1, T2>& data
)
{
  stream << data.first;
  stream << ":";
  stream << data.second;
    ...
    (__GNUC__ * 100 + __GNUC_MINOR__) >=        \
        405  // skip for GCC < 4.5 due to https://gcc.gnu.org/bugzilla/show_bug.cgi?id=38600
#if !defined(_MSC_VER) || \
    _MSC_VER >            \
        1400  // MSVC 2005 doesn't understand `enableIf`, so drop all `meta`
...
inline typename meta::enableIf<
    meta::isConvertibleToNString<T>::value, void>::type
operator<< (util::nostringstream& stream, const T& data)
{
  plog::detail::operator<< (
      stream, static_cast<util::nstring> (data)
  );
}
...
#ifdef __cplusplus_cli
inline void operator<< (
    util::nostringstream& stream, System::String ^ data
)
{
  cli::pin_ptr<const System::Char> ptr = PtrToStringChars (data);
  plog::detail::operator<< (
      stream, static_cast<const wchar_t*> (ptr)
  );
}
#endif
...
struct isStreamable
    : valueType<
          sizeof (operator<< (
              meta::declval<Stream>(), meta::declval<const T>()
          )) != sizeof (No)> {};

template <class Stream>
struct isStreamable<
    std::ios_base & PLOG_CDECL (std::ios_base&), Stream>
    : valueType<true> {};
...
class Record {
 public:
  Record (
      Severity severity, const char* func, size_t line,
      const char* file, const void* object, int instanceId
  )
      : m_severity (severity),
        m_tid (util::gettid()),
        m_object (object),
  ...
  Record& operator<< (util::nostream&(PLOG_CDECL* data) (
      util::nostream&
  ))
  {
    m_message << data;

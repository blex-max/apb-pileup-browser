...
class HexDump {
 public:
  ...
  HexDump& separator (
      const char* digitSeparator, const char* groupSeparator
  )
  {
    m_digitSeparator = digitSeparator;
  ...
  friend util::nostringstream& operator<< (
      util::nostringstream& stream, const HexDump&
  );
...
inline util::nostringstream& operator<< (
    util::nostringstream& stream, const HexDump& hexDump
)
{
  ...
  for (size_t i = 0; i < hexDump.m_size;) {
    stream << std::setw (2)
           << static_cast<int> (hexDump.m_ptr[i]);
...
inline HexDump hexdump (const Container& container)
{
  return HexDump (
      container.data(),
...
inline HexDump hexdump (const T (&arr)[N])
{

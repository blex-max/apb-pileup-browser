...
#if defined(PLOG_ENABLE_GET_THIS) && defined(_MSC_VER) && \
    _MSC_VER >= 1600 && !defined(__INTELLISENSE__) &&     \
    !defined(__INTEL_COMPILER) && !defined(__llvm__) &&   \
    !defined(                                             \
        __RESHARPER__                                     \
    )  // >= Visual Studio 2010, skip IntelliSense, Intel Compiler, Clang Code Model and ReSharper
...
#define PLOG_(instanceId, severity)                             \
  IF_PLOG_ (instanceId, severity) (*plog::get<instanceId>()) += \
      plog::Record (                                            \
          severity, PLOG_GET_FUNC(), __LINE__, PLOG_GET_FILE(), \
...
#define PLOG_VERBOSE_(instanceId) \
  PLOG_ (instanceId, plog::verbose)
#define PLOG_DEBUG_(instanceId) PLOG_ (instanceId, plog::debug)
#define PLOG_INFO_(instanceId) PLOG_ (instanceId, plog::info)
#define PLOG_WARNING_(instanceId) \
  PLOG_ (instanceId, plog::warning)
...
#define PLOG_VERBOSE_IF(condition) \
  PLOG_IF (plog::verbose, condition)
#define PLOG_DEBUG_IF(condition) PLOG_IF (plog::debug, condition)
#define PLOG_INFO_IF(condition) PLOG_IF (plog::info, condition)
#define PLOG_WARNING_IF(condition) \
  PLOG_IF (plog::warning, condition)
...
#define PLOGV_IF_(instanceId, condition) \
  PLOG_VERBOSE_IF_ (instanceId, condition)
#define PLOGD_IF_(instanceId, condition) \
  PLOG_DEBUG_IF_ (instanceId, condition)
#define PLOGI_IF_(instanceId, condition) \
  PLOG_INFO_IF_ (instanceId, condition)
#define PLOGW_IF_(instanceId, condition) \
  PLOG_WARNING_IF_ (instanceId, condition)
#define PLOGE_IF_(instanceId, condition) \
  PLOG_ERROR_IF_ (instanceId, condition)
#define PLOGF_IF_(instanceId, condition) \
  PLOG_FATAL_IF_ (instanceId, condition)
#define PLOGN_IF_(instanceId, condition) \
  PLOG_NONE_IF_ (instanceId, condition)
...
#define LOG_VERBOSE_IF(condition) \
  PLOG_IF (plog::verbose, condition)
...
#define LOGV_IF_(instanceId, condition) \
  PLOG_VERBOSE_IF_ (instanceId, condition)
#define LOGD_IF_(instanceId, condition) \
  PLOG_DEBUG_IF_ (instanceId, condition)
#define LOGI_IF_(instanceId, condition) \
  PLOG_INFO_IF_ (instanceId, condition)
#define LOGW_IF_(instanceId, condition) \
  PLOG_WARNING_IF_ (instanceId, condition)
#define LOGE_IF_(instanceId, condition) \
  PLOG_ERROR_IF_ (instanceId, condition)
#define LOGF_IF_(instanceId, condition) \
  PLOG_FATAL_IF_ (instanceId, condition)
#define LOGN_IF_(instanceId, condition) \
  PLOG_NONE_IF_ (instanceId, condition)
#endif

...
inline int vasprintf (
    char** strp, const char* format, va_list ap
)
{
  va_list ap_copy;
...
#else
  int retval = vsnprintf_s (
      str, bufferCharCount, static_cast<size_t> (-1), format, ap
  );
#endif
  if (retval < 0) {
    free (str);
...
#ifdef _WIN32
inline int vaswprintf (
    wchar_t** strp, const wchar_t* format, va_list ap
)
{
#if defined(__BORLANDC__)
  int charCount =
      0x1000;  // there is no _vscwprintf on Borland/Embarcadero
#else
  ...
  wchar_t* str = static_cast<wchar_t*> (
      malloc (bufferCharCount * sizeof (wchar_t))
  );
  if (!str) {
    return -1;
  }
...
#else
  int retval = _vsnwprintf_s (
      str, bufferCharCount, charCount, format, ap
  );
#endif
  if (retval < 0) {
    free (str);
...
inline std::wstring toWide (
    const char* str, UINT cp = codePage::kChar
)
{
  ...
  if (!wstr.empty()) {
    int wlen = MultiByteToWideChar (
        cp, 0, str, static_cast<int> (len), &wstr[0],
...
inline std::wstring toWide (
    const std::string& str, UINT cp = codePage::kChar
)
{
  return toWide (str.c_str(), cp);
}

inline const std::wstring& toWide (
    const std::wstring& str
)  // do nothing for already wide string
{
  return str;
}
...
inline std::string toNarrow (const std::wstring& wstr, long page)
{
  int len = WideCharToMultiByte (
      page, 0, wstr.c_str(), static_cast<int> (wstr.size()), 0,
      0, 0, 0
  );
  ...
  if (!str.empty()) {
    WideCharToMultiByte (
        page, 0, wstr.c_str(), static_cast<int> (wstr.size()),
        &str[0], len, 0, 0
    );
  }
...
inline std::string toNarrow (const wchar_t* wstr)
{
  ...
  if (!str.empty()) {
    ...
    ::iconv (
        cd, const_cast<char**> (&in), &inBytes, &out, &outBytes
    );
    ::iconv_close (cd);
...
inline void splitFileName (
    const nchar* fileName, nstring& fileNameNoExt,
    nstring& fileExt
)
{
 ...
 public:
  ...
  size_t open (const nstring& fileName)
  {
    ...
    m_file = ::_wsopen (
        ...
        _O_CREAT | _O_WRONLY | _O_BINARY | _O_NOINHERIT,
        SH_DENYWR, _S_IREAD | _S_IWRITE
    );
#elif defined(_WIN32)
    ::_wsopen_s (
        &m_file, toWide (fileName).c_str(),
        _O_CREAT | _O_WRONLY | _O_BINARY | _O_NOINHERIT,
...
#else
    m_file = ::open (
        fileName.c_str(), O_CREAT | O_APPEND | O_WRONLY,
        S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH
    );
#endif
  ...
  size_t seek (size_t offset, int whence)
  {
    ...
    (defined(__BORLANDC__) || defined(__MINGW32__))
                     ::_lseek (
                         m_file, static_cast<off_t> (offset),
                         whence
                     )
#elif defined(_WIN32)
                     ::_lseeki64 (
  ...
  static int rename (
      const nstring& oldFilename, const nstring& newFilename
  )
  {
#ifdef _WIN32
    return MoveFileW (
        toWide (oldFilename).c_str(),
        toWide (newFilename).c_str()
    );
#else

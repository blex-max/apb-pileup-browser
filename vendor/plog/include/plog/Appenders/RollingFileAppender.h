...
template <
    class Formatter,
    class Converter = NativeEOLConverter<UTF8Converter>>
class PLOG_LINKAGE_HIDDEN RollingFileAppender
    : public IAppender {
 public:
  RollingFileAppender (
  ...
  RollingFileAppender (
      const char* fileName, size_t maxFileSize = 0,
      int maxFiles = 0
  )
      : m_fileSize(),
        m_maxFileSize(),
  ...
  virtual void write (const Record& record) PLOG_OVERRIDE
  {
    ...
    size_t bytesWritten = m_file.write (
        Converter::convert (Formatter::format (record))
    );
  ...
  void setMaxFileSize (size_t maxFileSize)
  {
    m_maxFileSize =
        (std::max)(maxFileSize,
                   static_cast<size_t> (
                       1000
                   ));  // set a lower limit for the maxFileSize
  }
  ...
  void rollLogFiles()
  {
    ...
    for (int fileNumber = m_maxFiles - 2; fileNumber >= 0;
         --fileNumber) {
      util::nstring currentFileName = buildFileName (fileNumber);
      util::nstring nextFileName =
          buildFileName (fileNumber + 1);
